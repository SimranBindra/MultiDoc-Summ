#this is for generating summary of 200 words, threshold for inclusion of sentence in problem is 0.05, for inclusion of sentence in summary is 0.4 and target is to minimise summation of inverse_sentence_score*value(weighted) where sentence score is calculated by its words' importance, and threshold for putting a fragment in a cluster is 0.09,extra stop words included.
...
def set_cover(b,q,l,cluster,sentences,folder, sent_score):
	...
	for c in cluster :
		...
		for i in minisummary:
			...
			'''print(len(i[1].split(" ")))
			if a+len(i[1].split(" "))>no_of_words :
				print('now and a : ',no_of_words,a)
				break
			summary.append(i[1])
			a=a+len(i[1].split(" "))
			print(a)
			sentences.remove(i[1])'''
		#LD.write('\n\n\n\n\n')
	LD = open('simvipfragmenting/Summary7_6/'+folder +'_reference.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
...
if not os.path.exists("simvipfragmenting/Summary7_6"):
	os.mkdir("simvipfragmenting/Summary7_6")
