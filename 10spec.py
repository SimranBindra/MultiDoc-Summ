...
WORD = re.compile('\w+')
total_words=0
def ltt(matrix,n,sentences,frags,folder,dict1,frag_score):
	st2=time.time()
	s1=matrix
	lim=len(s1[0])
	for i in range(lim):
	...
	set_cover(b,n,m,cluster,sentences,folder, sent_score,order_cluster)
...
def extractSentences(sentences,frags,folder,dict1,dict2):
	...
	frag_score=[0 for _ in range(0,len(d))]
	for i in range(0,len(d)):
		frag_score[i]=0
		for j in range(0,len(d)):
			frag_score[i]+=d[j][i]
	print("...%s seconds..." %(time.time()-st1))
	ltt(d,n,sentences,frags,folder,dict1,frag_score)	
...
def set_cover(b,q,l,cluster,sentences,folder, sent_score,order_cluster):
	st3=time.time()
	...
	for c in order_cluster :
		...
		no_of_words=(n/float(q))*(100)
		a=0
		for i in minisummary:
			if y>=100:
				break
			if a+len(i[1].split(" "))>abs(no_of_words+x):
				break
	...
	LD = open('simvipfragmenting/Summary10spec/'+folder +'/summary.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
...
if not os.path.exists("simvipfragmenting/Summary10spec"):
	os.mkdir("simvipfragmenting/Summary10spec")
for folder in folders:
	if not os.path.exists("simvipfragmenting/Summary10spec/"+folder):
		os.mkdir("simvipfragmenting/Summary10spec/"+folder)
	print(folder)
	st=time.time()
