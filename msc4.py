#this is for generating summary of 200 bytes, threshold for inclusion of sentence in problem is 0.2, for inclusion of sentence in summary is 0.5 and target is to minimise summation of length*value of sentence(unweighted), and threshold for putting a fragment in a cluster is 0.07, no extra stop words included.
...
def set_cover(b,q,l,cluster,sentences,folder):
	...
	for c in cluster :
		...
		no_of_bytes=(n/float(q))*200
		#print(no_of_sentences,n,q,l)
		a=0
		for i in minisummary:
			if a+len(i)>abs(no_of_bytes+x+5):
				x=no_of_bytes+x-a
				break
			summary.append(i[1])
			a=a+len(i)
			sentences.remove(i[1])
	LD = open('simvipfragmenting/Summary2/'+folder +'_reference4.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
