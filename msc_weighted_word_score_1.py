#this is for generating summary of 200 words, threshold for inclusion of sentence in problem is 0.2, for inclusion of sentence in summary is 0.8 and target is to minimise summation of inverse_sentence_score*value(weighted) where sentence score is calculated by its words' importance, and threshold for putting a fragment in a cluster is 0.07,extra stop words included.
...
def set_cover(b,q,l,cluster,sentences,folder, sent_score):
	...
	for c in cluster :
		...
		for i in minisummary:
			...
			sentences.remove(i[1])'''
		#LD.write('\n\n\n\n\n')
	LD = open('simvipfragmenting/Summary7/'+folder +'_reference.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
...
if not os.path.exists("simvipfragmenting/Summary7"):
	os.mkdir("simvipfragmenting/Summary7")
