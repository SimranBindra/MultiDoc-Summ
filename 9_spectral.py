...
def set_cover(b,q,l,cluster,sentences,folder, sent_score):
	...
	LD = open('simvipfragmenting/Summary9_spectral/'+folder +'/summary.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
...
if not os.path.exists("simvipfragmenting/Summary9_spectral"):
	os.mkdir("simvipfragmenting/Summary9_spectral")
for folder in folders:
	if not os.path.exists("simvipfragmenting/Summary9_spectral/"+folder):
		os.mkdir("simvipfragmenting/Summary9_spectral/"+folder)
	print(folder)
	st=time.time()
