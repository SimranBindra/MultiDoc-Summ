#this is for generating summary of any size, threshold for inclusion of sentence in problem is 0.2, for inclusion of sentence in summary is 0.8
#and target is to minimise summation of length*value of sentence(unweighted), and threshold for putting a fragment in a cluster is 0.07, no extra stop words included.
...
def extractSentences(sentences,frags,folder,cluster,dict1):
	...
	with open('simvipfragmenting/TEscoresMonz2/'+folder+'score.txt','w') as LM:
		print('s1=[', end="\n", file=LM)
		for i in range(n):
			for j in range(m):
				print('{:.4f}'.format(float(d[i][j]))+' ', end="\n", file=LM)
		print('', end="\n", file=LM)
...
def set_cover(b,q,l,cluster,sentences,folder):
	...
	for c in cluster :
		...
		first = leng[0]*values[0]
		for i in range(1,m):
		    first = first + leng[i]*values[i]
		    prob +=first
		...
		no_of_sentences=(n/float(q))*(l/float(50))
		#print(no_of_sentences,n,q,l)
		a=0
		for i in minisummary:
			if a>=abs(no_of_sentences):
				break
			summary.append(i[1])
			a=a+1
			sentences.remove(i[1])
	LD = open('simvipfragmenting/Summary2/'+folder +'_reference2.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
