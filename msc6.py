#this is for generating summary of 200 words, threshold for inclusion of sentence in problem is 0.4, for inclusion of sentence in summary is 0.8 and target is to minimise summation of value of sentence, and threshold for putting a fragment in a cluster is 0.07, extra stop words included.
...
def extractSentences(sentences,frags,folder,cluster,dict1):
	...
	'''with open('C:/nlp project/simvipfragmenting/TEscoresMonz2/'+folder+'score.txt','w') as LM:
		...
		print>>LM,'\n'
	'''
	set_cover(d,n,m,cluster,sentences,folder)   
	#print("...%s seconds..." %((time.time()-st)/60))
	

def set_cover(b,q,l,cluster,sentences,folder):
	...
	for c in cluster :
		...
		for i in minisummary:
			#print('a is: ',a)
			if a+len(i[1].split(" "))>abs(no_of_words+x+5):
				print('no of words and a is:',no_of_words,a)
				x=no_of_words+x-a
				break
			summary.append(i[1])
			#print(len(i[1]))
	...
	LD = open('simvipfragmenting/Summary6/'+folder +'_reference6.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
...
if not os.path.exists("simvipfragmenting/TEscoresMonz6"):
	os.mkdir("simvipfragmenting/TEscoresMonz6")
if not os.path.exists("simvipfragmenting/Summary6"):
	os.mkdir("simvipfragmenting/Summary6")
for folder in folders:
	...
	#remove empty words to count the number of words
		words = nltk.word_tokenize(NFrags[i])
		for wor in spw :
			if wor in words :
				words.remove(wor)
		if(len(words) <= 2):
