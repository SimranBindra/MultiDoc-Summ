...
def extractSentences(sentences,frags,folder,dict1):
	...
	for pair in nodePairs:
	   ...
	   # print "\nWUP Scores- NOUNS\n"
		for i in wordTokens1:
			var1=float(1+n_containing(i,frags))
			if var1<=n :
				idf1= math.log(n/var1) #if den>num causes negative values
			else:
				idf1= math.log(n/float(n_containing(i,frags))) # or not to consider in idf1 since num will be =  to den 
			idf_Ti+=idf1
			if i in wordTokens2:
				idf_comm+=idf1
		for j in wordTokens2:
			var2=float(1+n_containing(j,frags))
			if var2<=n:
				idf2= math.log(n/var2)
			else:
				idf2= math.log(n/float(n_containing(j,frags)))
			idf_Tj+=idf2       
		for i in range(n):
...
def set_cover(b,q,l,cluster,sentences,folder, sent_score):
	...
	LD = open('simvipfragmenting/Summary9s/'+folder +'_reference.txt', 'w')
	for i in summary:
		LD.write(i+'\n')                
	LD.close();
	

def n_containing(word, frags):
	return sum(1 for frag in frags if word in frag)
...
if not os.path.exists("simvipfragmenting/Summary9s"):
	os.mkdir("simvipfragmenting/Summary9s")
	...
	for i in frags:
		...
		for j in words:
			...
			if(dict1.get(j)==None):
				...
				dict1[j]=dict1[j]+1
	extractSentences(sentences,frags,folder,dict1)
